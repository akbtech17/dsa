...
public:
    vector<vector<int>> levelOrder(TreeNode* root) {
        ...
        while(!q.empty()) {
        	...
        	ans[clevel].push_back(currnode->val);

        	if(currnode->left) q.push({currnode->left,clevel+1});
        	if(currnode->right) q.push({currnode->right,clevel+1});
        }
        return ans;
    }
};
